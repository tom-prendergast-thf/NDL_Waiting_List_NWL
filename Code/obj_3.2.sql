...
SELECT
    ...
    CASE
        WHEN
            DATEDIFF(DAY, rtt_start_date, rtt_end_date) / 7.0 BETWEEN 0 AND 18
            THEN '00-18'
        WHEN
            DATEDIFF(DAY, rtt_start_date, rtt_end_date) / 7.0 BETWEEN 18 AND 36
            THEN '18-36'
        WHEN
            DATEDIFF(DAY, rtt_start_date, rtt_end_date) / 7.0 BETWEEN 36 AND 52
            THEN '36-52'
        WHEN
            DATEDIFF(DAY, rtt_start_date, rtt_end_date) / 7.0 > 52
            THEN '52+'
        END AS wait_time_category_1,
    SUM(1) AS pathways,
    COUNT(DISTINCT patient_key) AS unique_patients,
    SUM(DATEDIFF(DAY, rtt_start_date, rtt_end_date) / 7.0) AS patient_weeks,
    SUM(em_3m_prior) AS events_prior,
